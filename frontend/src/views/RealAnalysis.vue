    ...
    <el-row :gutter="20" class="mb-4">
      <el-col :span="8">
        <el-card shadow="hover" class="stat-card">
          ...
          <div class="stat-sub">{{ stats.total_profit_pct?.toFixed(2) }}%</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover" class="stat-card">
...
const loadTrades = async () => {
  try {
    const res = await actual.getTrades()
    if (res.data.success) {
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        let html = `<div style="padding: 8px; font-weight: bold;">${week}</div>`
        params.forEach(p => {
          const color = p.seriesName === '上证指数' ? '#409EFF' : (p.value >= 0 ? '#f56c6c' : '#67c23a')
          const sign = p.value >= 0 ? '+' : ''
          html += `<div style="padding: 4px 8px;">
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        let html = `<div style="padding: 8px; font-weight: bold;">${month}月</div>`
        params.forEach(p => {
          const color = p.seriesName === '上证指数' ? '#409EFF' : (p.value >= 0 ? '#f56c6c' : '#67c23a')
          const sign = p.value >= 0 ? '+' : ''
          html += `<div style="padding: 4px 8px;">
